...
extern void UART1_Putstr(unsigned char *ptr);
...
void UserIO(void){					// interfata cu utilizatorul
	...
	if(tasta){
		...
		switch(STARE_IO){
			...
			if(cmd == '1' && tasta < NR_NODURI &&  tasta >='0' && tasta != ADR_NOD +'0')																// daca comanda e '1' si adresa e intre '0' - '4', mai putin adresa proprie
			{	
				dest	= tasta-'0';																						// extrage dest din tasta
					if(retea[dest].full != 0)												// Daca este deja un mesaj in buffer ...
						{
																													// afiseaza Buffer plin
							LCD_PutStr(1,0,"Buffer plin");
							UART1_Putstr("Buffer plin");
					...
					else																								// altfel ...
					{
						...
						retea[dest].bufbin.dest = dest;														// pune in bufferul dest adresa nodului sursa ADR_NOD
						retea[dest].bufbin.src = ADR_NOD;														// pune in bufferul dest adresa nodului sursa ADR_NOD
							...
							LCD_PutStr(1,0,"Introduce mesajul:");																										// cere introducerea mesajului
							UART1_Putstr("Introduce mesajul:");
						lng = 0;																								// initializeaza lng = 0 
						STARE_IO = 2;																						// trece in starea 2, sa astepte caracterele mesajului
					}			
			}												
			if(cmd == '2' && tasta < NR_NODURI &&  tasta >='0')																// daca comanda e '2' si adresa e intre '0'-'4'
			{	dest = tasta- '0';																							// extrage dest din tasta
				if(retea[dest].full != 0)																// Daca este deja un mesaj in buffer ...
					{
																												// Afiseaza Buffer plin
